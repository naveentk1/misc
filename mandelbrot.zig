...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    _ = &argv;
    if (argc != @as(c_int, 8)) {
        _ = printf("Usage:   %s <xmin> <xmax> <ymin> <ymax> <maxiter> <xres> <out.ppm>\n", argv[@as(c_uint, @intCast(@as(c_int, 0)))]);
        _ = printf("Example: %s 0.27085 0.27100 0.004640 0.004810 1000 1024 pic.ppm\n", argv[@as(c_uint, @intCast(@as(c_int, 0)))]);
        exit(@as(c_int, 1));
    }
    const xmin: f64 = atof(argv[@as(c_uint, @intCast(@as(c_int, 1)))]);
    _ = &xmin;
    const xmax: f64 = atof(argv[@as(c_uint, @intCast(@as(c_int, 2)))]);
    _ = &xmax;
    const ymin: f64 = atof(argv[@as(c_uint, @intCast(@as(c_int, 3)))]);
    _ = &ymin;
    const ymax: f64 = atof(argv[@as(c_uint, @intCast(@as(c_int, 4)))]);
    _ = &ymax;
    const maxiter: u16 = @as(c_ushort, @bitCast(@as(c_short, @truncate(atoi(argv[@as(c_uint, @intCast(@as(c_int, 5)))])))));
    _ = &maxiter;
    const xres: c_int = atoi(argv[@as(c_uint, @intCast(@as(c_int, 6)))]);
    _ = &xres;
    const yres: c_int = @as(c_int, @intFromFloat((@as(f64, @floatFromInt(xres)) * (ymax - ymin)) / (xmax - xmin)));
    _ = &yres;
    var filename: [*c]const u8 = argv[@as(c_uint, @intCast(@as(c_int, 7)))];
    _ = &filename;
    var fp: [*c]FILE = fopen(filename, "wb");
    _ = &fp;
    var comment: [*c]u8 = @as([*c]u8, @ptrCast(@volatileCast(@constCast("# Mandelbrot set"))));
    _ = &comment;
    _ = fprintf(fp, "P6\n# Mandelbrot, xmin=%lf, xmax=%lf, ymin=%lf, ymax=%lf, maxiter=%d\n%d\n%d\n%d\n", xmin, xmax, ymin, ymax, @as(c_int, @bitCast(@as(c_uint, maxiter))), xres, yres, if (@as(c_int, @bitCast(@as(c_uint, maxiter))) < @as(c_int, 256)) @as(c_int, 256) else @as(c_int, @bitCast(@as(c_uint, maxiter))));
    var dx: f64 = (xmax - xmin) / @as(f64, @floatFromInt(xres));
    _ = &dx;
    var dy: f64 = (ymax - ymin) / @as(f64, @floatFromInt(yres));
    _ = &dy;
    var x: f64 = undefined;
    _ = &x;
    var y: f64 = undefined;
    ...
    _ = &k;
    {
        j = 0;
        while (j < yres) : (j += 1) {
            y = ymax - (@as(f64, @floatFromInt(j)) * dy);
            {
                i = 0;
                while (i < xres) : (i += 1) {
                    var u_1: f64 = 0.0;
                    ...
                    var v2: f64 = v_2 * v_2;
                    _ = &v2;
                    x = xmin + (@as(f64, @floatFromInt(i)) * dx);
                    {
                        k = 1;
                        while ((k < @as(c_int, @bitCast(@as(c_uint, maxiter)))) and ((@"u2" + v2) < 4.0)) : (k += 1) {
                            v_2 = ((@as(f64, @floatFromInt(@as(c_int, 2))) * u_1) * v_2) + y;
                            u_1 = (@"u2" - v2) + x;
                            @"u2" = u_1 * u_1;
                            v2 = v_2 * v_2;
                        }
                    }
                    if (k >= @as(c_int, @bitCast(@as(c_uint, maxiter)))) {
                        const black: [6]u8 = [6]u8{
                            0,
                            0,
                            0,
                            0,
                        ...
                        _ = &black;
                        _ = fwrite(@as(?*const anyopaque, @ptrCast(@as([*c]const u8, @ptrCast(@alignCast(&black[@as(usize, @intCast(0))]))))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 6)))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), fp);
                    } else {
                        var color: [6]u8 = undefined;
                        _ = &color;
                        color[@as(c_uint, @intCast(@as(c_int, 0)))] = @as(u8, @bitCast(@as(i8, @truncate(k >> @intCast(8)))));
                        color[@as(c_uint, @intCast(@as(c_int, 1)))] = @as(u8, @bitCast(@as(i8, @truncate(k & @as(c_int, 255)))));
                        color[@as(c_uint, @intCast(@as(c_int, 2)))] = @as(u8, @bitCast(@as(i8, @truncate(k >> @intCast(8)))));
                        color[@as(c_uint, @intCast(@as(c_int, 3)))] = @as(u8, @bitCast(@as(i8, @truncate(k & @as(c_int, 255)))));
                        color[@as(c_uint, @intCast(@as(c_int, 4)))] = @as(u8, @bitCast(@as(i8, @truncate(k >> @intCast(8)))));
                        color[@as(c_uint, @intCast(@as(c_int, 5)))] = @as(u8, @bitCast(@as(i8, @truncate(k & @as(c_int, 255)))));
                        _ = fwrite(@as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&color[@as(usize, @intCast(0))]))))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 6)))), @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), fp);
                    }
                }
            }
        }
...
pub const __ENVIRONMENT_MAC_OS_X_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 150700, .decimal);
pub const __ENVIRONMENT_OS_VERSION_MIN_REQUIRED__ = @import("std").zig.c_translation.promoteIntLiteral(c_int, 150700, .decimal);
